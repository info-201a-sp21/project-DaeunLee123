...
summary_info <- function(agtable) {
  ...
  listed_summary <- list(mean_pop = round(mean_population, 0), mean_total =
                           round(mean_total_pplvac, 0),
                        avg_percent_pplpop = 100 * round(avg_percent_pplpop, 3),
                        mean_vac = round(mean_total_vac, 0), avg_percent_pplvac
                        = 100 * round(avg_percent_pplvac, 3))
